...
const INIT_SPOTS = [
  ...
  { id:5,  name:'Koh Samui Sunset',      city:'Koh Samui',   area:'Chaweng',     lat:9.5324,  lng:100.0675, type:'beach',      vibe:'Beach Vibes',     rating:4.5, reviews:203, description:'Beachfront area on Chaweng Beach with sunset views. Pre-rolls served with cocktails at golden hour.',                                  tags:['beach','sunset','island']        },
...
function makeMarker(spot, isSelected) {
  ...
  const pulse = isSelected
    ? `<div style="position:absolute;top:50%;left:50%;width:${s*3}px;height:${s*3}px;margin-left:-${s*1.5}px;margin-top:-${s*1.5}px;border-radius:50%;border:1.5px solid ${neon};opacity:.5;animation:pulse-ring 1.8s ease-out infinite;"></div>`
    : '';
  return L.divIcon({
    className: '',
    html: `<div style="position:relative;display:flex;align-items:center;justify-content:center;width:${s*3}px;height:${s*3}px;">${pulse}<div style="width:${s}px;height:${s}px;border-radius:50%;background:${neon};box-shadow:0 0 ${glow}px ${neon},0 0 ${glow*2}px ${neon}55;border:1.5px solid rgba(255,255,255,.2);"></div></div>`,
    iconSize:   [s*3, s*3],
...
function makePendingMarker() {
  return L.divIcon({
    className: '',
    html: `<div style="width:18px;height:18px;border-radius:50%;background:#ff6b35;box-shadow:0 0 16px #ff6b35,0 0 32px #ff6b3555;border:2px solid rgba(255,255,255,.2);animation:pulse-ring 1s ease-out infinite;"></div>`,
...
export default function App() {
  ...
  // Draw markers — only when filtered list changes
  useEffect(() => {
    const map = mapInst.current;
    if (!map) return;
  ...
  // Update marker icons when selection changes — no full redraw
  useEffect(() => {
    selectedId.current = selected?.id ?? null;
    markersRef.current.forEach(({ id, marker }) => {
      const spot = spots.find(s => s.id === id);
      if (!spot) return;
      marker.setIcon(makeMarker(spot, id === selected?.id));
  ...
  // ── AI send ── calls /api/chat proxy on Vercel ────────────────────────────
  const sendAI = async () => {
  ...
  return (
    <div style={{ height:'100vh', display:'flex', flexDirection:'column', background:BG, fontFamily:"'DM Mono',monospace", overflow:'hidden', color:TEXT1 }}>
      ...
      <div style={{ flex:1, position:'relative', overflow:'hidden' }}>
        ...
        {/* Add mode banner */}
        {addMode && (
          <div style={{ position:'absolute', top:12, left:'50%', transform:'translateX(-50%)', zIndex:500, background:BG2, border:'1px solid #ff6b35', borderRadius:5, padding:'7px 16px', fontSize:10, color:'#ff6b35', letterSpacing:'.08em', textTransform:'uppercase', boxShadow:'0 0 18px #ff6b3533', pointerEvents:'none', whiteSpace:'nowrap' }}>
            {pendingPos ? `📍 ${pendingPos.lat.toFixed(4)}, ${pendingPos.lng.toFixed(4)} — fill details below` : '◉ TAP THE MAP TO PIN YOUR SPOT'}
          </div>
        )}

        {/* Bottom cards */}
        {!selected && !addMode && (
          <div style={{ position:'absolute', bottom:0, left:0, right:0, zIndex:100, background:`linear-gradient(transparent,${BG} 38%)`, paddingTop:36 }}>
              ...
              {filtered.map(spot => {
                ...
                return (
                  <div key={spot.id} onClick={()=>selectSpot(spot)}
                    style={{ flexShrink:0, width:190, background:BG2, border:`1px solid ${BORDER}`, borderRadius:8, padding:13, cursor:'pointer', position:'relative', overflow:'hidden' }}>
        ...
        {selected && !addMode && (() => {
          ...
          return (
            <div className="fade" style={{ position:'absolute', bottom:0, left:0, right:0, zIndex:100, background:BG2, borderTop:`1px solid ${neon}55`, borderRadius:'14px 14px 0 0', padding:'14px 18px 36px', maxHeight:'60%', overflowY:'auto' }}>
        ...
        {addMode && pendingPos && !submitted && (
          <div className="fade" style={{ position:'absolute', bottom:0, left:0, right:0, zIndex:100, background:BG2, borderTop:'1px solid #ff6b3566', borderRadius:'14px 14px 0 0', padding:'14px 18px 36px', maxHeight:'65%', overflowY:'auto' }}>
            <div style={{ width:30, height:3, background:BORDER, borderRadius:2, margin:'0 auto 14px' }} />
        ...
        {/* Submit success */}
        {addMode && submitted && (
          <div className="fade" style={{ position:'absolute', bottom:0, left:0, right:0, zIndex:100, background:BG2, borderTop:`1px solid ${NEON}44`, borderRadius:'14px 14px 0 0', padding:'32px 24px 48px', textAlign:'center' }}>
      ...
      {aiOpen && (
        <div className="fade" style={{ position:'fixed', inset:0, zIndex:3000, background:BG, display:'flex', flexDirection:'column' }}>
          <div style={{ padding:'12px 16px', borderBottom:`1px solid ${BORDER}`, display:'flex', alignItems:'center', gap:10, flexShrink:0 }}>
            <div style={{ width:32, height:32, borderRadius:4, background:`${NEON}18`, border:`1px solid ${NEON}55`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:15, color:NEON, animation:'glow 2s infinite' }}>◈</div>
            <div>
          ...
          <div style={{ flex:1, overflowY:'auto', padding:14, display:'flex', flexDirection:'column', gap:10 }}>
            {msgs.map((m,i) => (
              <div key={i} style={{ alignSelf:m.role==='user'?'flex-end':'flex-start', maxWidth:'85%' }}>
                <div style={{ padding:'9px 13px', borderRadius:m.role==='user'?'8px 8px 2px 8px':'8px 8px 8px 2px', background:m.role==='user'?`${NEON}1a`:BG2, border:`1px solid ${m.role==='user'?NEON+'33':BORDER}`, color:TEXT1, fontSize:12, lineHeight:1.7 }}>
                  {m.content}
                </div>
              </div>
            ))}

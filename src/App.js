...
// ─── MAPTILER KEY ─────────────────────────────────────────────────────────────
// Get a free key at maptiler.com → replace the string below
...
const INIT_SPOTS = [
  ...
  { id:7,  name:'The Green Room BKK',    city:'Bangkok',     area:'Thonglor',    lat:13.7298, lng:100.5839, type:'lounge',     vibe:'Luxury Lounge',   rating:4.8, reviews:290, description:'Luxury members lounge in Thonglor. Curated strains, live jazz, dim lighting. The most refined spot in Bangkok.', tags:['luxury','members','jazz'] },
...
// ─── MARKERS ──────────────────────────────────────────────────────────────────
function makeMarker(spot, selected = false) {
  const neon = TYPE_CFG[spot.type]?.neon || NEON;
  const s = selected ? 16 : 10;
  const pulse = selected ? `<div style="position:absolute;top:50%;left:50%;width:${s*3}px;height:${s*3}px;margin-left:-${s*1.5}px;margin-top:-${s*1.5}px;border-radius:50%;border:1.5px solid ${neon};opacity:0.5;animation:pulse-ring 1.8s ease-out infinite;"></div>` : '';
  return L.divIcon({
    className: '',
    html: `<div style="position:relative;display:flex;align-items:center;justify-content:center;width:${s*3}px;height:${s*3}px;">${pulse}<div style="width:${s}px;height:${s}px;border-radius:50%;background:${neon};box-shadow:0 0 ${selected?18:8}px ${neon},0 0 ${selected?36:16}px ${neon}55;border:1.5px solid rgba(255,255,255,0.2);"></div></div>`,
    iconSize: [s*3, s*3],
...
function makePendingMarker() {
  return L.divIcon({
    className: '',
    html: `<div style="width:18px;height:18px;border-radius:50%;background:#ff6b35;box-shadow:0 0 16px #ff6b35,0 0 32px #ff6b3555;border:2px solid rgba(255,255,255,0.2);animation:pulse-ring 1s ease-out infinite;"></div>`,
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const useMapTiler = MAPTILER_KEY !== 'YOUR_MAPTILER_KEY';
    L.tileLayer(
      useMapTiler
  ...
  useEffect(() => {
    ...
    if (!map) return;
    const onClick = (e) => {
      if (addMode) setPendingPos({ lat: e.latlng.lat, lng: e.latlng.lng });
    };
  ...
  useEffect(() => {
    ...
    if (pendingPos) pendingRef.current = L.marker([pendingPos.lat, pendingPos.lng], { icon: makePendingMarker() }).addTo(map);
  }, [pendingPos]);
  ...
  useEffect(() => {
    ...
    filtered.forEach(spot => {
      ...
      const m = L.marker([spot.lat, spot.lng], { icon: makeMarker(spot, isSelected), zIndexOffset: isSelected ? 1000 : 0 }).addTo(map);
  ...
  const sendAI = async () => {
    ...
    setMsgs(next); setAiLoading(true);
    ...
    try {
      const r = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          system:`You are the AI guide for 420 Spots Thailand. Be chill, concise, use emojis. Cannabis legal medicinally in Thailand.\n\nSpots:\n${ctx}`,
  ...
  const submitSpot = () => {
    ...
    setSpots(p => [...p, { id:Date.now(), ...form, lat:pendingPos.lat, lng:pendingPos.lng, rating:0, reviews:0, vibe:'New Spot', tags:[] }]);
    setSubmitted(true);
  };

  const closeAdd = () => {
    setAddMode(false); setPendingPos(null); setSubmitted(false);
    ...
    <div style={{ height:'100vh', display:'flex', flexDirection:'column', background:BG, fontFamily:"'DM Mono',monospace", overflow:'hidden', color:TEXT1 }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .nbtn{background:transparent;border:1px solid ${NEON};color:${NEON};font-family:'DM Mono',monospace;font-size:10px;letter-spacing:.08em;padding:6px 13px;border-radius:3px;cursor:pointer;transition:all .15s;text-transform:uppercase;white-space:nowrap;flex-shrink:0}
        ...
        .idk{width:100%;background:${BG3};border:1px solid ${BORDER};color:${TEXT1};padding:9px 13px;border-radius:5px;font-size:12px;font-family:'DM Mono',monospace;outline:none;transition:border-color .15s}
      ...
      {/* TOP BAR */}
      <div style={{ background:BG, borderBottom:`1px solid ${BORDER}`, zIndex:20, flexShrink:0 }}>
        <div style={{ padding:'10px 14px 8px', display:'flex', alignItems:'center', gap:8 }}>
          <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:20, letterSpacing:'.1em', color:NEON, lineHeight:1, flexShrink:0 }}>
            420 SPOTS<span style={{ fontSize:10, color:TEXT2, fontFamily:"'DM Mono',monospace", marginLeft:7, letterSpacing:'.04em' }}>TH</span>
          </div>
          <input className="idk tap" value={search} onChange={e=>setSearch(e.target.value)} placeholder="search spots, cities..." style={{ flex:1, padding:'7px 12px' }} />
          <button className="tap" onClick={()=>setShowKey(p=>!p)}
            style={{ width:30,height:30,borderRadius:4,background:apiKey?`${NEON}22`:BG3,border:`1px solid ${apiKey?NEON+'55':BORDER}`,color:apiKey?NEON:TEXT2,fontSize:13,display:'flex',alignItems:'center',justifyContent:'center' }}>🔑</button>
          <button className="tap" onClick={()=>setAiOpen(true)}
            style={{ width:30,height:30,borderRadius:4,background:BG3,border:`1px solid ${BORDER}`,color:NEON,fontSize:14,display:'flex',alignItems:'center',justifyContent:'center' }}>◈</button>
          <button className={`nbtn tap${addMode?' on':''}`}
        ...
        {showKey && (
          <div style={{ padding:'0 14px 10px', display:'flex', gap:8 }}>
            <input className="idk" type="password" value={apiKey} onChange={e=>setApiKey(e.target.value)} placeholder="sk-ant-... Anthropic API key" style={{ flex:1 }} />
            <button className="nbtn tap on" onClick={()=>{localStorage.setItem('420_api_key',apiKey);setShowKey(false)}}>SAVE</button>
          </div>
        )}
        ...
        <div style={{ display:'flex', gap:6, padding:'0 14px 10px', overflowX:'auto' }}>
          {TYPES.map(t => {
            ...
            return (
              <button key={t} className={`nbtn tap${isOn?' on':''}`}
      ...
      <div style={{ flex:1, position:'relative', overflow:'hidden' }}>
        ...
        {/* Add mode banner */}
        {addMode && (
          <div style={{ position:'absolute',top:12,left:'50%',transform:'translateX(-50%)',zIndex:800,background:BG2,border:`1px solid #ff6b35`,borderRadius:5,padding:'7px 16px',fontSize:10,color:'#ff6b35',letterSpacing:'.08em',textTransform:'uppercase',boxShadow:'0 0 18px #ff6b3533',pointerEvents:'none',whiteSpace:'nowrap' }}>
            {pendingPos ? `📍 ${pendingPos.lat.toFixed(4)}, ${pendingPos.lng.toFixed(4)} — fill details below` : '◉ TAP MAP TO PIN YOUR SPOT'}
          </div>
        )}

        {/* BOTTOM SPOT CARDS */}
        {!selected && !addMode && (
          <div style={{ position:'absolute',bottom:0,left:0,right:0,background:`linear-gradient(transparent,${BG} 38%)`,paddingTop:36 }}>
            ...
            <div className="cards">
              {filtered.map(spot => {
                const neon = TYPE_CFG[spot.type]?.neon || NEON;
                return (
                  <div key={spot.id} onClick={()=>{setSelected(spot);mapInst.current?.flyTo([spot.lat,spot.lng],14,{duration:.7})}}
                    style={{ flexShrink:0,width:190,background:BG2,border:`1px solid ${BORDER}`,borderRadius:8,padding:13,cursor:'pointer',transition:'all .15s',position:'relative',overflow:'hidden' }}>
                    <div style={{ position:'absolute',top:0,left:0,right:0,height:'2px',background:neon,opacity:.8 }} />
                    ...
                    <div style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:14,letterSpacing:'.05em',color:TEXT1,marginBottom:2,lineHeight:1.2 }}>{spot.name}</div>
                    <div style={{ fontSize:9,color:TEXT2,marginBottom:7 }}>{spot.city} · {spot.area}</div>
                    <div style={{ fontSize:9,color:TEXT2,lineHeight:1.5,display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical',overflow:'hidden' }}>{spot.description}</div>
                  </div>
                );
              })}
            </div>
        ...
        {selected && !addMode && (()=>{
          ...
          return (
            <div className="fade" style={{ position:'absolute',bottom:0,left:0,right:0,background:BG2,borderTop:`1px solid ${neon}44`,borderRadius:'14px 14px 0 0',padding:'14px 18px 36px',maxHeight:'60%',overflowY:'auto',boxShadow:`0 -6px 40px ${BG}dd` }}>
              ...
              <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:12 }}>
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:22,letterSpacing:'.06em',color:TEXT1,lineHeight:1.1,marginBottom:3 }}>{selected.name}</div>
                  <div style={{ fontSize:10,color:TEXT2 }}>📍 {selected.city} · {selected.area}</div>
                </div>
                <button className="tap" onClick={()=>setSelected(null)}
                  style={{ background:BG3,border:`1px solid ${BORDER}`,borderRadius:4,width:28,height:28,fontSize:13,color:TEXT2,display:'flex',alignItems:'center',justifyContent:'center' }}>✕</button>
              </div>
              ...
              <div style={{ display:'flex',alignItems:'center',gap:10,padding:'10px 13px',background:BG3,border:`1px solid ${BORDER}`,borderRadius:7,marginBottom:14 }}>
                ...
                <div>
                  <div style={{ fontSize:12,color:'#ffd93d',letterSpacing:2 }}>{'★'.repeat(Math.round(selected.rating))}{'☆'.repeat(5-Math.round(selected.rating))}</div>
                  <div style={{ fontSize:9,color:TEXT3 }}>{selected.reviews>0?`${selected.reviews} reviews`:'Be the first!'}</div>
                </div>
              ...
              {selected.tags?.length>0 && (
                <div style={{ display:'flex',flexWrap:'wrap',gap:5,marginBottom:14 }}>
                  {selected.tags.map(tag=>(
                    <span key={tag} style={{ fontSize:9,padding:'2px 8px',borderRadius:3,background:`${neon}18`,color:neon,letterSpacing:'.08em',textTransform:'uppercase',border:`1px solid ${neon}33` }}>#{tag}</span>
                  ))}
                </div>
              )}

              <button className="nbtn tap on" style={{ width:'100%',padding:'11px',fontSize:11,textAlign:'center',borderColor:neon,background:`${neon}18`,color:neon }}
        ...
        {addMode && pendingPos && !submitted && (
          <div className="fade" style={{ position:'absolute',bottom:0,left:0,right:0,background:BG2,borderTop:'1px solid #ff6b3555',borderRadius:'14px 14px 0 0',padding:'14px 18px 36px',maxHeight:'65%',overflowY:'auto' }}>
            <div style={{ width:30,height:3,background:BORDER,borderRadius:2,margin:'0 auto 14px' }} />
            <div style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:20,letterSpacing:'.08em',color:'#ff6b35',marginBottom:3 }}>NEW SPOT</div>
            <div style={{ fontSize:9,color:TEXT3,marginBottom:14,letterSpacing:'.06em' }}>📍 {pendingPos.lat.toFixed(5)}, {pendingPos.lng.toFixed(5)}</div>
            <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:9,marginBottom:9 }}>
              {[['name','Spot Name *'],['city','City'],['area','Area']].map(([k,l])=>(
                <div key={k} style={k==='name'?{gridColumn:'1/-1'}:{}}>
                  <div style={{ fontSize:8,color:TEXT3,marginBottom:4,letterSpacing:'.1em',textTransform:'uppercase' }}>{l}</div>
                  <input className="idk" value={form[k]} onChange={e=>setForm(p=>({...p,[k]:e.target.value}))} placeholder={k==='name'?'e.g. Green House':k==='city'?'Bangkok':'Sukhumvit'} />
                </div>
              ))}
              <div>
            ...
            <div style={{ display:'flex',gap:9 }}>
              <button className="nbtn tap" style={{ flex:1,padding:'10px',borderColor:'#ff6b35',color:BG,background:'#ff6b35',boxShadow:'0 0 14px #ff6b3555',opacity:form.name?1:.5 }}
                onClick={submitSpot} disabled={!form.name}>◉ PIN SPOT</button>
        ...
        {/* SUBMIT SUCCESS */}
        {addMode && submitted && (
          <div className="fade" style={{ position:'absolute',bottom:0,left:0,right:0,background:BG2,borderTop:`1px solid ${NEON}44`,borderRadius:'14px 14px 0 0',padding:'32px 24px 48px',textAlign:'center' }}>
            <div style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:44,color:NEON,marginBottom:6,textShadow:`0 0 28px ${NEON}` }}>PINNED!</div>
            <div style={{ fontSize:11,color:TEXT2,marginBottom:24,lineHeight:1.7 }}>Your spot is live on the map.<br/>Thanks for growing the community 🌿</div>
            <button className="nbtn tap on" onClick={closeAdd} style={{ padding:'11px 32px' }}>BACK TO MAP</button>
          </div>
        )}
      </div>
      ...
      {aiOpen && (
        <div className="fade" style={{ position:'fixed',inset:0,zIndex:3000,background:BG,display:'flex',flexDirection:'column' }}>
          <div style={{ padding:'12px 16px',borderBottom:`1px solid ${BORDER}`,display:'flex',alignItems:'center',gap:10,flexShrink:0 }}>
            <div style={{ width:32,height:32,borderRadius:4,background:`${NEON}18`,border:`1px solid ${NEON}55`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:15,animation:'glow 2s infinite',color:NEON }}>◈</div>
            <div>
            ...
            <button className="tap" onClick={()=>setAiOpen(false)}
              style={{ marginLeft:'auto',background:BG3,border:`1px solid ${BORDER}`,borderRadius:4,width:28,height:28,fontSize:13,color:TEXT2,display:'flex',alignItems:'center',justifyContent:'center' }}>✕</button>
          </div>

          <div style={{ flex:1,overflowY:'auto',padding:14,display:'flex',flexDirection:'column',gap:10,background:BG }}>
            {msgs.map((m,i)=>(
              <div key={i} style={{ alignSelf:m.role==='user'?'flex-end':'flex-start',maxWidth:'85%' }}>
                <div style={{ padding:'9px 13px',borderRadius:m.role==='user'?'8px 8px 2px 8px':'8px 8px 8px 2px',background:m.role==='user'?`${NEON}1a`:BG2,border:`1px solid ${m.role==='user'?NEON+'33':BORDER}`,color:TEXT1,fontSize:12,lineHeight:1.7 }}>{m.content}</div>
              </div>
            ))}
            {aiLoading && (
              <div style={{ alignSelf:'flex-start' }}>
                <div style={{ padding:'10px 14px',borderRadius:'8px 8px 8px 2px',background:BG2,border:`1px solid ${BORDER}`,color:NEON,fontSize:14,animation:'glow 1s infinite',letterSpacing:4 }}>◈◈◈</div>
              </div>
            )}
            <div ref={msgEnd} />
          </div>
          ...
          <div style={{ padding:'10px 14px 28px',display:'flex',gap:8,background:BG,borderTop:`1px solid ${BORDER}` }}>
            <input className="idk" value={aiInput} onChange={e=>setAiInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&sendAI()} placeholder="Ask anything about 420 spots in Thailand..." style={{ flex:1,borderRadius:4 }} />
            <button className="nbtn tap on" onClick={sendAI} disabled={aiLoading||!aiInput.trim()} style={{ padding:'9px 14px',opacity:aiInput.trim()?1:.4 }}>SEND</button>
          </div>
        </div>
      )}
    </div>
